...
public class HudComponent {
	...
	private float component_min_width;

	private float component_min_height;
		...
		if (this.component_inResizeState) {
			...
			while (n2 < n) {
				...
				if (node != null) {
					...
					fillrenderer.rect(node.getX(), node.getY(), 10.0f, 10.0f);
				}
				++n2;
			}
			linerenderer.setColor(Color.RED);
			for(HudNode node: component_nodes){
				if(node.getType() == HudNode.NODE_BOTTOM_LEFT){
					linerenderer.rect(node.getX() + (HudComponent.COMPONENT_RESIZE_NODE_SIZE / 2), node.getY()+ (HudComponent.COMPONENT_RESIZE_NODE_SIZE / 2), getWidth(), getHeight());
				}
				}
			}
	...
	public final void setWidth(float width) {
		if(component_min_width == 0){
			this.component_width = width;
			return;
		}
		if(width < component_min_width){
			this.component_width = component_min_width;
			return;
		}
		this.component_width = width;
	}
	...
	public final void setHeight(float height) {
		if(component_min_height == 0){
			this.component_height = height;
			return;
		}
		if(height < component_min_height){
			this.component_height = component_min_height;
			return;
		}
		this.component_height = height;
	}
	...
	public final void updateControlNodes(){
		for(HudNode n: getNodes()){
			n.updatePosition();
		}
	}
	...
	public final void setMinimumWidth(float width){
		component_min_width = width;
	}
	public final void setMinimumHeight(float height){
		component_min_height = height;
	}
